...
def _parse_state(state):
    '''
    we only use eef dim here

    0:3 right eef pose, abs
    3:6 right eef axis-angle, abs
    6:18 right hand qpos, abs
    18: 21 left eef pose, abs
    21: 24 left eef axis-angle, abs
    24: 36 left hand qpos, abs
    '''
    return torch.concat([state[0:6],state[18:24]])

def _parse_action(action):
    '''
    we only use eef dim here

    0:3 right eef pose, delta
    3:6 right eef axis-angle, delta
    6:12 right hand qpos, delta
    12:15 left eef pose, delta
    15:18 left eef axis-angle, delta
    18:24 left hand qpos, delta
    '''
    return torch.concat([action[:,0:6],action[:,12:18]],dim=1)
...
class DexmgInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        state = _parse_state(data["observation/state"])
        ...
        if "actions" in data:
            ...
            actions = _parse_action(data["actions"])
