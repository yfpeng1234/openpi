...
class LeRobotDexmgDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[dexmg_policy.DexmgInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi0_dexmg_low_mem_finetune",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotDexmgDataConfig(
            repo_id="physical-intelligence/task1",
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                prompt_from_task=True,
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader(os.getenv("OPEN_PI0_BASE_MODEL")),
        num_train_steps=30_000,
        # The freeze filter defines which parameters should be frozen during training.
        # We have a convenience function in the model config that returns the default freeze filter
        # for the given model config for LoRA finetuning. Just make sure it matches the model config
        ...
        project_name='openpi_dexmg',
        checkpoint_base_dir=os.getenv("OPENPI_CHECKPOINT_DIR"),
        num_workers=2,
        overwrite=True,
    ),
    TrainConfig(
